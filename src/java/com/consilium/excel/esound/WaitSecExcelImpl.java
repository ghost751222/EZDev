...
@Service("WaitSecExcelImpl")
public class WaitSecExcelImpl implements ExcelInterface {
...
//            String eDataTime = eDataTime_YYYY + eDataTime_MM + eDataTime_DD;
            ...
            try {
                ...
                idx = 0;
                sqlQuery = " select  convert(varchar(20),calltime,120)  ct, " +
                        " m.callclid, c.actionid, m.callwaitingduration " +
                        " from m7480_call m left join  calllog_form c  on m.m7480callid = c.m7480callid " +
                        " where m.calltime >= ? " +
                        " and m.calltime <= ? " +
                        " and m.answertime is not null " +
                        " order by m.calltime ";
                ...
                while (rs.next()) {
                    ...
                    obj.setCallclid(rs.getString("callclid"));
                    obj.setActionid(rs.getString("actionid"));
                    obj.setCallwaitingduration(rs.getString("callwaitingduration"));
                    resultList.add(obj);
                }
                rs.close();
                rs = null;
                ...
                String path = httpServletRequest.getServletContext().getRealPath("/WEB-INF/report/esound/WaitSec.xls");
