 ...
 *
 *
 *     File name:       FAQStatisticExcelImpl.java   (FAQ統計表)
 *
 *     History:
 *     Date               Author                  Comments
 *     -----------------------------------------------------------------------
 *     Jun 24, 2011      Mars               Initial Release
 *     Dec 23, 2014      Mars               增加分數欄位
 *****************************************************************************/
...
@Service("FAQStatisticExcelImpl")
public class FAQStatisticExcelImpl implements ExcelInterface {
    @Autowired
...
//        String endDate=eDataTime_YYYY + "-" + eDataTime_MM + '-' + eDataTime_DD;
            ...
            try {
                ...
                for (int i = 0; i < unitCodeOrder.length; i++) {
                    if (dataTable.containsKey(unitCodeOrder[i])) {
                        ...
                        } else if ("E2000A".equals(obj.getUnitCode())) {
                            obj.setUnitName("土地重劃工程處");
                        } else if ("E2000B".equals(obj.getUnitCode())) {
            ...
            String path = httpServletRequest.getServletContext().getRealPath("/WEB-INF/report/esound/FAQReport.xls");
