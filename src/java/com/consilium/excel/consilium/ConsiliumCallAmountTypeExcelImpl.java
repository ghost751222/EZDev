...
@Service("ConsiliumCallAmountTypeExcelImpl")
public class ConsiliumCallAmountTypeExcelImpl implements ExcelInterface {

    private static final Logger logger = Logger.getLogger(ConsiliumCallAmountTypeExcelImpl.class);
    private String excelName = "業務類型分析.xls";
    private String reportPath = "/WEB-INF/report/consilium/CallAmountType.xls";
    ...
    public ResponseEntity<byte[]> exportExcel(JsonNode jsonNode) throws Exception {
        ...
        if ("Y".equals(timeType)) {
            ...
            sqlQuery =
                    " SELECT SERVICETYPE,SERVICEITEM ,CALLTYPE," +
                            ...
                            " GROUP BY                                                      " +
                            "   SERVICETYPE,SERVICEITEM,CALLTYPE,  datepart(yyyy,CREATETIME)                     " +
                            " ORDER BY                                                      " +
            ...
            sqlQuery =
                    " SELECT SERVICETYPE,SERVICEITEM ,CALLTYPE," +
                           ...
                           // " and caseType is not null and caseType <> 'V'" +
                            ...
                            " GROUP BY                                                             " +
                            "   SERVICETYPE,SERVICEITEM,CALLTYPE,  datepart(yyyy,CREATETIME) , RIGHT(CONCAT('0', ISNULL(datepart(MM,CREATETIME),'')), 2) " +
                            " ORDER BY                                                             " +
        ...
        } else if ("W".equals(timeType)) // 邊撈邊塞日期索引
        {
            ...
            sqlQuery =
                    " SELECT                                                        " +
                            ...
                            "    GROUP BY                                                   " +
                            "       SERVICETYPE,SERVICEITEM,CALLTYPE, concat(convert(char(10),CREATETIME,126),'-',DATEPART(iso_week, CREATETIME))        " +
//                            "    ORDER BY                                                   " +
//                            "       SERVICETYPE, concat(convert(char(10),CREATETIME,126),'-',DATEPART(wk, CREATETIME))  )" +
            ...
            sqlQuery =
                    " SELECT SERVICETYPE,SERVICEITEM ,CALLTYPE," +
                            ...
                            " GROUP BY                                                      " +
                            "   SERVICETYPE,SERVICEITEM,CALLTYPE,  datepart(yyyy,CREATETIME), RIGHT(CONCAT('0', ISNULL(datepart(MM,CREATETIME),'')), 2) ,(datepart(DD,CREATETIME) /7 ) + 1,RIGHT(CONCAT('0', ISNULL(datepart(DD,CREATETIME),'')), 2)" +
                            " ORDER BY                                                      " +
            ...
            sqlQuery =
                    " SELECT SERVICETYPE,SERVICEITEM ,CALLTYPE," +
                            ...
                            " GROUP BY                                                      " +
                            "   SERVICETYPE,SERVICEITEM,CALLTYPE,  concat(FORMAT(DATEPART(HOUR,createTime),'00'),'~',FORMAT(DATEPART(HOUR,createTime)+1,'00'))  ,CALLTYPE                    " +
                            " ORDER BY                                                      " +
            ...
            sqlQuery =
                    " SELECT SERVICETYPE,SERVICEITEM ,CALLTYPE," +
                            ...
                            " GROUP BY                                                      " +
                            "   SERVICETYPE ,SERVICEITEM  ,CALLTYPE                 " +
                            " ORDER BY                                                      " +
                            "   SERVICETYPE                    ";
        }
        ...
        try {
            ...
            logger.info("業務類型分析 sql=" + sqlQuery);
            int j = 0;
            ...
            while (rs.next()) {
                ...
                serviceItem = rs.getString("SERVICEITEM");
                if ("A4007".equals(serviceItem)) whichType = "6";
                ...
                if ("Y".equals(timeType)) {
                    ...
                    if (colName.contains(whichType)) {
                        ...
                        //report.addInt((String) hp.get(rs.getString("YEAR") + rs.getString("MONTH")), appCode + "Z", rs.getInt("CNT"));
                    }
...
//                        }
                    ...
                    if (colName.contains(whichType)) {
                        ...
                        //report.addInt((String) hp.get(rs.getString("YEAR") + rs.getString("MONTH") + rs.getString("DAY")), appCode + "Z", rs.getInt("CNT"));
                    }
                    report.addInt((String) hp.get(rs.getString("YEAR") + rs.getString("MONTH") +
